...
def _campaign(prefix: str, eligible_at: int = 100) -> dict:
    return {
...
async def test_legacy_user_notification_masks_are_ignored(tmp_path):
    ...
    with sqlite3.connect(path) as connection:
        connection.execute(
            ...
            "notification_mask INTEGER NOT NULL DEFAULT 7)"
        )
        connection.executemany(
            "INSERT INTO users (user_id, locale, is_active, notification_mask) "
            "VALUES (?, 'uz', ?, ?)",
            [(11, 1, 0), (22, 1, 1), (33, 0, 0)],
        )

    db = Storage(str(path))
    await db.init()
    assert await db.get_active_user_ids() == [11, 22]
    assert await db.get_active_user_upper_bound() == 22
    async with db._db.execute(
        "SELECT notification_mask FROM users ORDER BY user_id"
    ) as cursor:
        assert [row[0] for row in await cursor.fetchall()] == [0, 1, 0]
    await db.close()
...
async def test_music_transaction_cannot_be_committed_by_unrelated_write(
    ...
    publication = asyncio.create_task(db.publish_music_collection(
        ...
        refresh_seconds=604800,
        campaign=_campaign("new_music", eligible_at=200),
    ))
...
async def test_outbox_ignores_legacy_masks_and_enforces_week_quota():
    db = Storage(":memory:")
    await db.init()
    # Simulate the users table left by the preference-enabled Railway release.
    ...
    await db._db.commit()
    assert await db.get_active_user_ids() == [11, 22, 33]

    for index, key in enumerate(("new_music", "rising", "discoveries")):
        ...
        await db.publish_music_collection(
            ...
            claim_token=claim,
            refresh_seconds=604800,
            campaign=_campaign(key, eligible_at=100 + index),
        )

    async with db._music_db.execute(
        "SELECT notification_mask FROM music_campaign_outbox ORDER BY campaign_id"
    ) as cursor:
        assert [row[0] for row in await cursor.fetchall()] == [1, 1, 1]
    # A queued row written by the previous release can retain any historical
    # category value; it is now compatibility data, never an audience filter.
    await db._music_db.execute(
        "UPDATE music_campaign_outbox SET notification_mask = 4 "
        "WHERE campaign_id = 1"
    )
    await db._music_db.commit()
    ...
    assert await db.checkpoint_music_campaign(
        first["campaign_id"], first["runner_token"], 22, "sent"
    )
    assert await db.checkpoint_music_campaign(
...
async def test_reclaimed_legacy_campaign_expands_ceiling_without_rewind():
    db = Storage(":memory:")
    await db.init()
    for user_id in (11, 44):
        await db.touch_private_user(user_id)
    await db.ensure_music_collection("new_music", 5)
    claim = await db.claim_music_collection_refresh(
        "new_music", 100, force=True
    )
    assert claim
    await db.publish_music_collection(
    ...
    sending = await db.claim_next_music_campaign(
        now=100, week_key="2026-W01"
    )
    assert sending is not None
    await db.release_music_campaign(
        sending["campaign_id"], sending["runner_token"]
    )
    await db._music_db.execute(
        "UPDATE music_campaign_outbox SET audience_upper_user_id = 11, "
        "broadcast_cursor = 11 WHERE campaign_id = ?",
        (sending["campaign_id"],),
    )
    await db._music_db.commit()

    resumed = await db.claim_next_music_campaign(
        now=101, week_key="2026-W01"
    )
    assert resumed is not None
    assert resumed["campaign_id"] == sending["campaign_id"]
    assert resumed["audience_upper_user_id"] == 44
    assert resumed["broadcast_cursor"] == 11
    await db.close()
